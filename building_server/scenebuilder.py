...
class SceneBuilder():
    ...
    def build(cls, cursor, city, layer, representations, maxDepth, tile, startingDepth):
        ...
        # Build hierarchy
        if maxDepth == None and startingDepth == None:
            query = "SELECT tile, child FROM {0}".format(tileHierarchy)
        else:
            query = "SELECT {0}.tile, child FROM {0} JOIN {1} ON {0}.child={1}.tile WHERE ".format(tileHierarchy, tileTable)
            if maxDepth != None:
                query += " depth<={0}".format(maxDepth)
                if startingDepth != None:
                    query += " AND"
            if startingDepth != None:
                query += " depth>={0}".format(startingDepth)
